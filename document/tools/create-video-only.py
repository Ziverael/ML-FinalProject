...
def getReleases(path):
    ...
    releases.sort()
    releases = map(
        lambda x: path + "master-release-" + str(x) + "/blind.png", releases
    )
    releases = list(releases)
...
parser = argparse.ArgumentParser(
    description="Create a Video from a GitHub Release Download of tobiasgrosser/paper-template."
)
parser.add_argument(
    "repo", help="owner/repo, e.g., tobias-grosser/paper-template"
)
