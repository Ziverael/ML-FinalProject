...
def createImageOfPaper(path, width=7, pages=21):
    ...
    densityWidth = (
        100 / float(pdfSize[0]) * (targetPage[0] / grid[0]) / 100 * 72
    )
    densityHeight = (
        100 / float(pdfSize[1]) * (targetPage[1] / grid[1]) / 100 * 72
    )
    ...
    for i in range(pageNumber, pages):
        cmd = [
            "convert",
            "-size",
            str(1458 / rows) + "x" + str(density * 10),
            "xc:white",
            name + "-tmp-" + str(i) + ".png",
        ]
        run(cmd, check=False)
    ...
    cmd = ["convert"]
    for row in range(math.ceil(pages / width)):
        cmd.append(name + "-tmp-row-" + str(row) + ".png")
    cmd.append("-append")
...
def getReleases(path):
    ...
    releases.sort()
    releases = map(
        lambda x: path + "master-release-" + str(x) + "/blind.pdf", releases
    )
    releases = list(releases)
...
def createVideo(path="", basename="paper"):
    run(["rm", path + "video.mp4"], check=False)
    cmd = [
        "ffmpeg",
        "-r",
        "2",
        ...
        "glob",
        "-i",
        path + f"*-files/{basename}.pdf-overlay.png",
        "-vcodec",
        "libx264",
        "-crf",
        "25",
        ...
        "yuv420p",
        "-vf",
        "scale=3840:2160:force_original_aspect_ratio=1,pad=3840:2160:(ow-iw)/2:(oh-ih)/2:white,format=rgb24",
        path + "video.mp4",
    ]
    run(cmd, check=False)
...
def plotStatistics(commit, filename, branch, count):
    ...
    ax.bar(
        date_ticks, histogram_highlight, align="center", alpha=1, color="red"
    )
    ax = plt.gca()
    ...
    title.text(
        0,
        0,
        time.strftime("%a, %d %b %H:%M", time.gmtime(commit.committed_date)),
    ...
    title.text(
        1,
        0,
        str(commit_pos) + "/" + str(commits_to_visit) + "         ",
...
def createImage(data):
    ...
    run(cmd, check=False, shell=True, stdout=DEVNULL, stderr=STDOUT)
    ...
    run(
        [
            ...
            "SouthEast",
            dirname + f"/{base_filename}.pdf-expanded.png",
            dirname + "/statistics.png",
            "-composite",
            dirname + f"/{base_filename}.pdf-overlay.png",
        ],
        check=False,
    )
...
if __name__ == "__main__":
    ...
    createImages(
        args.branch, args.count, args.filename, int(args.procs), args.make
    )
