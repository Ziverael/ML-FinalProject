...
On the other hand, an interesting example to consider may be the \(\Psi\)-Net described by Kuang et al.~\cite{Kuang2020} for the prediction of intracerebral haemorrhage.
...
To establish a reference point, the U-net implementation proposed by Tung Dinh~\cite{github.unet} was utilized. This model and the developed implementations were retrained on two distinct datasets. The first dataset comprised 100 satellite images and \(1\,000\) artificially generated images~\cite{kaggle.dataset.image_road}. The second dataset was the DeepGlobe 2018 Dataset~\cite{Demir2018}, which contains \(6\,226\) satellite images.

Three metrics were used for evaluation. The first was accuracy.
\[
acccuracy=\frac{TP+TN}{N},
\]
where \(TP\) are true positive, \(TN\) true negative and \(N\) number of classified objects. Next, Intersection over Union (IoU) calculates the amount of overlapping area between two bounding boxes - predicted bounding box and ground truth. The last one is Dice coefficient which is also similarity metric, but is more preferable in imbalanced datasets.

Training and evaluation yielded several notable observations. The models demonstrated higher accuracy values on the DeepGlobe dataset, with accuracy ranging from 0.96 to 0.98 across training steps. In contrast, the Kaggle dataset produced accuracy values between 0.9 and 0.92. This advantage is also reflected in the dice coefficient metric, although the distinction between datasets is less visually apparent. Additionally, models employing binary cross-entropy as a loss function outperformed those using dice coefficient loss for both metrics considered.

Fig.~\ref{fig:metrics} demonstrates that U-Net slightly outperforms the other two models in terms of the dice coefficient, accuracy, and intersection over union (IoU) metrics. The baseline model performs worse in all metrics except for the IoU coefficient, where it surpasses \(\Psi\)-Net. Regarding loss, the \(\Psi\)-Net model exhibits the most unstable convergence. The test data results are consistent with those observed in the validation data and are presented in Table~\ref{tab:test}. Notably, there is a substantial difference in training times: the baseline model required approximately 1 hour, U-Net required 3 hours, and \(\Psi\)-Net required 12 hours. Given these metric results, the \(\Psi\)-Net model does not justify the increased computational cost without further improvements.
...
\begin{figure}[htp]
    ...
    \caption{Validation metrics comparison for all three models with dice loss on the DeepGlobe dataset. Those models are U-Net (blue, alias unet\_base\_1\_dice\_dg), dedicated U-Net (green, alias unet\_modified\_1\_dice\_dg), and \(\Psi\)-Net (red, psinet\_dice\_dg).}
    \label{fig:metrics}
\end{figure}
...
From Fig.~\ref{fig:preds}, it is unclear which model performed best. On the one hand, the baseline seems to find roads most accurately, but on the other hand, it classified many more random objects as road fragments. Furthermore, the road lines in its case are the thickest. The other two models generated thinner lines, as if they were trying to focus on finding the centreline of the roads. In this comparison, \(\Psi\)-Net performed the worst. It seemed very sparing in indicating roads.


\begin{figure}[htp]
    \centering
    \includegraphics[width=12cm]{comparisons.png}
    \caption{Comparison of prediction results for three different satellite images using trained models.}
    \label{fig:preds}
\end{figure}
...
\section{Conclusion}
The paper analyses the potential applications of the U-Net architecture for road recognition. A basic model with modified numbers of neurons and reductions in redundant layers was proposed. It was also decided to investigate the \(\Psi\)-Net model in this application. The models were tested on two different datasets and with two different loss functions: binary cross-entropy and dice coefficient loss. The proposed models achieved an accuracy of 0.9719 and 0.971 and a dice coefficient of 0.6521 and 0.6529 for Dedicated U-Net and \(\Psi\)-Net, respectively, which were better results than the default U-Net. However, a comparison of the actual road masks obtained suggests that the models require further improvement. In particular, \(\Psi\)-Net does not seem to be a particularly encouraging model for training, due to the fact that its training took four times longer than for the Dedicated U-Net.
