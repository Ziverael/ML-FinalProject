...
from constans import COLOR_INT_RANGE, IMG_EXT, IMG_SHAPE, PROJECT_PATHS
...
CONFIG_DEEP_GLOBE = DataConfig(
    source="DeepGlobe", label_ext="png", image_ext="jpg"
)

CONFIG_KAGGLE_SATELITE = DataConfig(
    source="KaggleSatelite", label_ext="jpg", image_ext="jpg"
)
...
def load_image_and_label(
    filename: str, config: DataConfig
) -> tuple[ImageMatrix, ImageMatrix]:
    filename, _ext = filename.split(".")
    img_path = (
        PROJECT_PATHS.data
        / f"{config.source}/image/{filename}.{config.image_ext}"
    )
    ...
    lab_path = (
        PROJECT_PATHS.data
        / f"{config.source}/label/{filename}.{config.label_ext}"
    )
    if not lab_path.is_file():
        msg = f"{lab_path} does not exist."
        raise ValueError(msg)
    return load_img(
        img_path, target_size=IMG_SHAPE, color_mode="rgb"
    ), load_img(lab_path, target_size=IMG_SHAPE, color_mode="grayscale")
...
if __name__ == "__main__":
    ...
    try:
        invalid_config = DataConfig(
            source="KaggleSatelite", label_ext="png", image_ext="jpeg"
        )
    ...
    try:
        invalid_config = DataConfig(
            source="KaggleSatelite", label_ext="svg", image_ext="jpg"
        )
        msg = "Not captured validation error"
        raise RuntimeError(msg)
    except ValueError:
