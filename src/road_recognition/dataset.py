...
class Dataset:
    def __init__(
        ...
        self.filenames: list[str] = [
            p.stem + p.suffix for p in self.image_files
        ]
        self.train_data: SLRawData | None = None
        self.val_data: SLRawData | None = None
        self.test_data: SLRawData | None = None

        self._load_image_and_label = partial(
            load_image_and_label, config=config
        )
    ...
    ) -> tuple[list[ImageNormedMatrix], list[ImageNormedMatrix]]:
...
if __name__ == "__main__":
    ...
    print(
        f"Train: {len(ds.train_data.x) / n:.2f}; Test: {len(ds.test_data.x) / n:.2f}; Val: {len(ds.val_data.x) / n:.2f}"
    )
    print(type(ds_gen))
    iter_max = 10
